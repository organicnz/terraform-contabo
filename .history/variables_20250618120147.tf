...
variable "storage_size_limit" {
  description = "Storage size limit in GB for object storage"
  type        = number
  default     = 250
  
  validation {
    condition     = var.storage_size_limit > 0 && var.storage_size_limit <= 10000
    error_message = "Storage size limit must be between 1 and 10000 GB."
  }
}
